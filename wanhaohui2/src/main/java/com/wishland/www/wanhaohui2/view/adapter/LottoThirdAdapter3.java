...
public class LottoThirdAdapter3 extends SuperAdapter<CaipiaoBean.ItemsBean.SubvideoBean> {
    ...
    public LottoThirdAdapter3(Activity ctx, Model model, UserSP userSP, Handler handler) {
        super(ctx);
    ...
    protected void onBindItemView(View view, int viewType, int position) {
        ...
        CaipiaoBean.ItemsBean.SubvideoBean subgameBean = getItem(position);
...
//        FrescoUtil.loadAllGifPicOnNet(simpleDraweeView, "res://com.wishland.www.wanhaohui2/" + R.drawable.beijing11_5);
    ...
    private void setCBClickListener(LinearLayout llCollect, final CheckBox cb, final CaipiaoBean.ItemsBean.SubvideoBean subgameBean, final int position) {
        ...
        llCollect.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (userSP.getInt(UserSP.LOGIN_SUCCESS) == 1) {
                    ...
                    model.skipLoginActivity(activity, LoginActivity.class, "LottoThirdAdapter3");
                }
            }
        });
    }

    private void cancelCollectGame(final CheckBox cb, final CaipiaoBean.ItemsBean.SubvideoBean subgameBean, final int position) {
    ...
    private void collectGame(final CheckBox cb, final CaipiaoBean.ItemsBean.SubvideoBean subgameBean, final int position) {
        final Message message = mHandler.obtainMessage();
        message.what = 1;
        String token = userSP.getToken("token");
        Map<String, String> map = new HashMap<>();
